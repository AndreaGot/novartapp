...
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
var sc_code_ver="H.25.4_050413"
var s_account="seatpgwebagpgprod,seatpgglobalprod" // NOTE - Include appropriate RSID value, start with development then CHANGE it FOR TEST AND PRODUCTION
var s_spg=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
/* Conversion Config */
s_spg.currencyCode="EUR"
s_spg.charSet="UTF-8";
/* Link Tracking Config */
//s_spg.useForcedLinkTracking = false
s_spg.trackDownloadLinks=true
s_spg.trackExternalLinks=true
if(s_account.indexOf("seatpgwebagpg")>-1||s_account.indexOf("seatpgwebagpgcomm")>-1||s_account.indexOf("seatpgwebagpb")>-1||s_account.indexOf("seatpgwebagmob")>-1) s_spg.trackExternalLinks=false
s_spg.trackInlineStats=true
s_spg.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx"
s_spg.linkInternalFilters="javascript:,."  // NOTE - Remove the dot and include internal domains list
s_spg.linkLeaveQueryString=false
s_spg.linkTrackVars="None"
s_spg.linkTrackEvents="None"
...
2019:'3/31,10/27'}
/* Plugin Config */
s_spg.usePlugins=true
function s_spg_doPlugins(s_spg) {
	/* Add calls to plugins here */
	/* Set Page View Event */
  s_spg.events=s_spg.apl(s_spg.events,'event2',',',2)
  
  /*Campaign variable logic*/
  if(!s_spg.campaign)s_spg.campaign=s_spg.getQueryParam('excmp_id');
  s_spg.campaign=s_spg.getValOnce(s_spg.campaign, 'sc_cmp', 30);
  
  /* Set Server value w domain */
  if(!s_spg.server){
	s_spg.server=s_spg.wd.location.hostname?s_spg.wd.location.hostname:'No Hostname';
	// s.server=(""+s.wd.location).toLowerCase();
	}
  
  /* Copy props in eVars */
  if(s_spg.pageName&&!s_spg.eVar19) s_spg.eVar19='D=pageName';
  if(s_spg.channel&&!s_spg.eVar20) s_spg.eVar20='D=ch';
  if(s_spg.prop1&&!s_spg.eVar1) s_spg.eVar1='D=c1';
  if(s_spg.prop2&&!s_spg.eVar2) s_spg.eVar2='D=c2';
  if(s_spg.prop8&&!s_spg.eVar8) s_spg.eVar8='D=c8';
  if(s_spg.prop10&&!s_spg.eVar10) s_spg.eVar10='D=c10';
  
  /* Hierarchies */
   if(!s_spg.hier1&&s_spg.channel){
	s_spg.hier1=s_spg.channel; {
	if(s_spg.hier1) 
		for(var i=1;i<7;i++) {
			if(s_spg['prop'+i]) s_spg.hier1=s_spg.apl(s_spg.hier1,s_spg['prop'+i],',',0);
		}
	}
  }
  
  if(!s_spg.hier2&&s_spg.hier1&&s_spg.prop8){
	s_spg.hier2=s_spg.prop8+','+s_spg.hier1;
  }
  
  /* If content type is Schede Aziende fire event4 */	
  if(s_spg.prop10=='Schede Aziende') s_spg.events+=",event4"; 
  
  /* Set Time Parting Variables - CET */
  var tpA = s_spg.getTimeParting('n','+1');
  s_spg.prop11=tpA //use classifications to group the time and the day of week into weekends
  if(s_spg.prop11&&!s_spg.eVar11) s_spg.eVar11='D=c11';
  s_spg.prop12=tpA[1] // Set hour
  s_spg.prop13=tpA[2]; // Set day
  s_spg.prop14=tpA[3]; // Set weekend

  /* Get days since last visit */
  s_spg.prop15=s_spg.eVar15=s_spg.getDaysSinceLastVisit('s_lv');  
    
  /* Get new vs returning */
  s_spg.prop16=s_spg.eVar16=s_spg.getNewRepeat();
  
  // Include s_vi or visitorID depending on availability of visitorID
  if (s_spg.visitorID) {s_spg.eVar18=s_spg.visitorID} else s_spg.eVar18="D=s_vi";
  
  /* Get previous Values */
  s_spg.prop24=s_spg.getPreviousValue(s_spg.pageName,'gpv_p24','');

  /* If error page collect URL value */	
  if(s_spg.pageType=='errorPage'){
	s_spg.prop25='D=g';
    s_spg.events+=",event3"; 
  }
	
  /* Provide RSID source */
  if(!s_spg.prop42) s_spg.prop42=s_account;  
  /* Provide code version change notes detail number */
  if(sc_code_ver){s_spg.prop43=sc_code_ver;} else {s_spg.prop43='unknown';}
  
/* Override URL and referrer for Web agencies and PGcomm sites */
  if(s_account.indexOf("seatpgwebagpgcomm")>-1){
	s_spg.linkInternalFilters="javascript:,seatpgwebagpgcomm.it";
	s_spg.pageURL='http://www.seatpgwebagpgcomm.it/?'+document.URL;
	if (!s_spg.referrer&&document.referrer&&s_spg.prop24&&s_spg.prop24.indexOf(s_spg.channel)>-1) s_spg.referrer='http://www.seatpgwebagpgcomm.it/?'+document.referrer;
	}
  else if(s_account.indexOf("seatpgwebagpb")>-1){
	s_spg.linkInternalFilters="javascript:,seatpgwebagpb.it";
	s_spg.pageURL='http://www.seatpgwebagpb.it/?'+document.URL;
	if (!s_spg.referrer&&document.referrer&&s_spg.prop24&&s_spg.prop24.indexOf(s_spg.channel)>-1) s_spg.referrer='http://www.seatpgwebagpb.it/?'+document.referrer;
	}
  else if(s_account.indexOf("seatpgwebagmob")>-1){
	s_spg.linkInternalFilters="javascript:,seatpgwebagmob.it";
	s_spg.pageURL='http://www.seatpgwebagmob.it/?'+document.URL;
	if (!s_spg.referrer&&document.referrer&&s_spg.prop24&&s_spg.prop24.indexOf(s_spg.channel)>-1) s_spg.referrer='http://www.seatpgwebagmob.it/?'+document.referrer;
	}
  else if(s_account.indexOf("seatpgwebagpg")>-1){
	s_spg.linkInternalFilters="javascript:,seatpgwebagpg.it";
	s_spg.pageURL='http://www.seatpgwebagpg.it/?'+document.URL;
	if (!s_spg.referrer&&document.referrer&&s_spg.prop24&&s_spg.prop24.indexOf(s_spg.channel)>-1) s_spg.referrer='http://www.seatpgwebagpg.it/?'+document.referrer;
	}	

}
s_spg.doPlugins=s_spg_doPlugins
/************************** PLUGINS SECTION *************************/
/* You may insert any plugins you wish to use here.                 */
/*
 * Plugin Utility: Replace v1.0
 */
s_spg.repl=new Function("x","o","n",""
+"var i=x.indexOf(o),l=n.length;while(x&&i>=0){x=x.substring(0,i)+n+x."
+"substring(i+o.length);i=x.indexOf(o,i+l)}return x");
/*
 * Plugin: getNewRepeat 1.2 - Returns whether user is new or repeat
 */
s_spg.getNewRepeat=new Function("d","cn",""
...
/*
 * Plugin: getTimeParting 3.3 - Set timeparting values based on time zone
 */
s_spg.getTimeParting=new Function("h","z",""
 ...
 */
s_spg.getQueryParam = new Function("p", "d", "u", ""
+ "var s=this,v='',i,t;d=d?d:'';u=u?u:(s.pageURL?s.pageURL:s.wd.locati"
+ "on);if(u=='f')u=s.gtfs().location;while(p){i=p.indexOf(',');i=i<0?p"
+ ".length:i;t=s.p_gpv(p.substring(0,i),u+'');if(t){t=t.indexOf('#')>-"
+ "1?t.substring(0,t.indexOf('#')):t;}if(t)v+=v?d+t:t;p=p.substring(i="
+ "=p.length?i:i+1)}return v");
s_spg.p_gpv = new Function("k", "u", ""
+ "var s=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"
+ "=s.pt(q,'&','p_gvf',k)}return v");
s_spg.p_gvf = new Function("t", "k", ""
 ...
 */
s_spg.getPreviousValue=new Function("v","c","el",""
 ...
 */
s_spg.getValOnce=new Function("v","c","e","t",""
 ...
 */
s_spg.getDaysSinceLastVisit=new Function("c",""
 ...
 */
s_spg.split=new Function("l","d",""
+"var i,x=0,a=new Array;while(l){i=l.indexOf(d);i=i>-1?i:l.length;a[x"
+"++]=l.substring(0,i);l=l.substring(i+d.length);}return a");
/*
 * Plugin Utility: apl v1.1
 */
s_spg.apl=new Function("l","v","d","u",""
...
s_spg.visitorNamespace="seatpg"
s_spg.trackingServer="metrics.seat.it"
s_spg.trackingServerSecure="smetrics.seat.it"
